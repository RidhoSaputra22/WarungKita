<x-admin-app>
    ...
    <form action="{{ route('menu.update', $data['222339_id_menu']) }}" method="POST" class="py-5 flex " enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="w-full">
            ...
            <div class="w-full h-auto mb-2">
                ...
                <input name="harga" value="{{ $data['222339_harga'] }}" class="rounded w-full border p-2">
            </div>
            <div class="w-full h-auto mb-2">
                <h1 class=" text-sm">Stok</h1>
                <input type="number" name="stok"  value="{{ $data['222339_stok'] }}"  class="rounded w-full border p-2">
            </div>
            <div class="w-full h-auto mb-2">
                <h1 class=" text-sm">Kategori</h1>
                <select name="kategori" id="" class="w-full px-2 py-3 border">
                    @foreach ($kategori as $k )
                        <option value="{{$k['222339_id_kategori']}}" {{ ($k['222339_id_kategori'] == $data['222339_id_kategori']) ? 'selected' : '' }} >{{ $k['kategori'] }}</option>
                    @endforeach
                </select>
            </div>
            <div class="w-full h-auto mb-2">
